...
@export var distribution_radius := 100.0 # (float, 10.0, 10000.0)
...
func _generate_shapes_exact():
	...
	while count_since_last_join < polygons.size() and runaway_counter < 200:
		...
		while i < polygons.size():
			var poly = polygons[i]
			var intersection = Geometry.intersect_polygons(curr_poly, poly)
			if intersection:
				polygons.erase(poly)
				curr_poly = Geometry.merge_polygons(curr_poly, poly)[0]
...
func _generate_shapes_simple():
	...
	while _carving_polygons and loop_count < loop_limit:
		...
		while collision_polygons:
			...
			var intersection = Geometry.intersect_polygons(carve_poly, col_poly)
			if !intersection:
				new_collsion_polygons.append(col_poly)
				continue
			var clip_results = Geometry.clip_polygons(col_poly, carve_poly)
			var have_hole := false
